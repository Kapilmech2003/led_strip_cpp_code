...
DMXController::DMXController(const char* port, int numGroups) 
    : portName(port), totalChannels(numGroups * 4), 
      totalLEDs(numGroups * LEDS_PER_GROUP), totalGroups(numGroups) {
...
void DMXController::sendDMXFrame() {
    if (!setBaudRate(90000)) throw std::runtime_error("Failed to set break baud rate");
    ...
    if (!setBaudRate(250000)) throw std::runtime_error("Failed to set data baud rate");
...
int main() {
    try {
        ...
        while (true) {
            int effectCode;
            std::cout << "Enter effect code (1001: solid red, 1002: solid blue, 1003: solid green, 1004: breathing green, 1005: breathing red, 1006: flashing red, 1007: flashing blue, -1 to exit): ";
            std::cin >> effectCode;

            if (effectCode == -1) break;
            ...
            switch (effectCode) {
                ...
                case 1007:
                    ...
                    break;
                case 1008:
                    effects.startEffect("flashing", 0, 128, 0, 0); // Flashing Blue
                    break;
                case 1009:
